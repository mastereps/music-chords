...
export function ChecklistRow({ item, onStatusChange, onAttemptsChange, onNotesChange, onDelete, onConfirmReview }: ChecklistRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        ...
        <StatusSelect value={item.status} onChange={onStatusChange} label={`Status for ${item.name}`} />
      </div>

      <div className="flex flex-col gap-1">
        <FieldLabel>Attempts</FieldLabel>
        <AttemptsStepper value={item.attempts} onChange={onAttemptsChange} label={`Attempts for ${item.name}`} />
      </div>
